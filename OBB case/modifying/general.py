...
from utils.nms_rotated.nms_rotated_wrapper  import obb_soft_nms
...
def soft_non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=1500):
    """Runs soft_Non-Maximum Suppression (NMS) on inference results_obb
    Args:
        prediction (tensor): (b, n_all_anchors, [cx cy l s obj num_cls theta_cls])
        agnostic (bool): True = NMS will be applied between elements of different categories
        labels : () or
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        rboxes[:, :2] = rboxes[:, :2] + c # rboxes (offset by class)
        scores = x[:, 5]  # scores
        _, i = obb_soft_nms(rboxes, scores, iou_thres, device_id = x.device)
        if i.shape[0] > max_det:  # limit detections
            i = i[:max_det]
