...
from utils.general import (LOGGER, box_iou, check_dataset, check_img_size, check_requirements, check_yaml,
                           ...
                           scale_coords, scale_polys, xywh2xyxy, xyxy2xywh, non_max_suppression_obb, soft_non_max_suppression_obb)
...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        filename_023 = paths[0].split('/')[-1].split('.')[0]
        t1 = time_sync()
        ...
        # out = soft_non_max_suppression_obb(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls) #soft nms
        out = non_max_suppression_obb(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls) # list*(n, [xylsθ, conf, cls]) θ ∈ [-pi/2, pi/2) # nms
        dt[2] += time_sync() - t3
        ...
        # Plot images
        if plots and batch_i < 645:
            f = save_dir / f'{filename_023}_Val_labels.jpg'  # labels
            Thread(target=plot_images, args=(im, targets, paths, f, names), daemon=True).start()
            f = save_dir / f'{filename_023}_Val_pred.jpg'  # predictions
